...
if ($size === 'xl') $dialogClass .= ' modal-xl';
    ...
    <div class="mb-2">
        ...
        <?php if ($type === 'textarea'): ?>
            <textarea
                ...
                <?= $req ? 'required' : '' ?>
                <?= $readonly ? 'readonly' : '' ?>
                rows="3"><?= esc((string)($val ?? '')) ?></textarea>
...
<div class="modal fade" id="<?= esc($modalId) ?>" tabindex="-1" aria-labelledby="<?= esc($modalId) ?>Label" aria-hidden="true">
    <div class="<?= esc($dialogClass) ?> modal-dialog-centered">
        <div class="modal-content">
            <form id="<?= esc($formId) ?>" data-modal-form data-api="<?= esc($api) ?>" data-method="<?= esc($method) ?>">
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-lg-6">
                            <?php foreach ($leftFields as $f) $renderField($f); ?>
                        </div>
                        <div class="col-12 col-lg-6">
                            <?php foreach ($rightFields as $f) $renderField($f); ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-sm btn-primary">
                        <i class="fa-solid fa-floppy-disk me-1"></i> <?= esc($submitText) ?>
                    </button>
                </div>
            </form>
        </div>

        ...
        <div class="d-flex gap-2">
            <form id="searchForm" class="d-flex">
                <input type="text" class="form-control form-control-sm" name="q"
                    placeholder="Cari material/deskripsi..." style="width:250px;">
    ...
    async function loadPredictData(page = 1, q = "") {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                tbody.innerHTML = `<tr><td colspan="8" class="text-muted">Tidak ada data ditemukan.</td></tr>`;
                return;
            }

            let i = (meta.page - 1) * meta.per_page + 1;
            for (const d of data) {
                const safetyStock = Math.round(d.safety_stock);
                const eoq = Math.round(d.eoq);
                const currentStock = Math.round(d.current_stock);
                const days = Math.round(d.days_until_order);
                let status = d.order_status;

                if (days < 0 && status.includes("hari")) {
                    status = "Stok Aman";
                }

                let badgeClass = "bg-secondary";
                if (status.includes("Sekarang")) badgeClass = "bg-danger";
                else if (status.includes("Aman")) badgeClass = "bg-success";
            ...
            for (let i = start; i <= end; i++) {
                pagHTML += `<li class="page-item ${i === meta.page ? "active" : ""}">
